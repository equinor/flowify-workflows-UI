...
export const sortByYouth: Sorter = (a: Resource, b: Resource) => b.metadata.creationTimestamp?.localeCompare(a.metadata.creationTimestamp ?? '') ?? -1;
...
export class ListWatch<T extends Resource> {
    ...
    private readonly sorter: ((a: T, b: T) => number) | undefined;
    ...
    public start() {
        ...
        this.list()
            .then(x => {
                this.items = x.items;// (x.items || []).sort(this.sorter);
 ...
 */
const mergeItem = <T extends Resource>(item: T, type: Type, items: T[] | undefined): T[] => {
    if (!items) return [];
