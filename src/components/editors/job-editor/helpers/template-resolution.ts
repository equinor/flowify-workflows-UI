...
export function getResolvedTemplates(workflow: models.Workflow, node: models.NodeStatus): models.Template | undefined {
  let tmpTemplate = {
  ...
  let resolvedTemplate: models.Template | undefined = undefined;
  const maxDepth = 10;
  for (let i = 1; i < maxDepth + 1; i++) {
    const templRef = resolveTemplateReference(
      scope.ResourceScope,
      scope.ResourceName!,
      tmpTemplate,
...
function getTemplateScope(nodeStatus: models.NodeStatus): {
  CompatibilityMode: boolean;
